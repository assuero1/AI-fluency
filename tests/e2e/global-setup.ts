...
// The QA fixture itself is created by the webServer command
// (scripts/start-e2e-server.mjs), which Playwright starts before globalSetup.
async function createAuthState(baseURL: string) {
  ...
  try {
    // config.use (serviceWorkers: "block") does NOT apply to browsers created
    // manually in globalSetup — without this the PWA sw.js reloads the page and
    // wipes the filled form.
