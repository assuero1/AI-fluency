...
const practiceSessionIds = new Set(records.TEABLE_PRACTICE_SESSIONS_TABLE_ID.filter((record) => userIds.has(record.fields?.user_id)).map((record) => record.id));
const flashcardIds = new Set(records.TEABLE_FLASHCARDS_TABLE_ID.filter((record) => practiceSessionIds.has(record.fields?.practice_session_id)).map((record) => record.id));
